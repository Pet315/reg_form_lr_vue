<template>
    <form @submit.prevent="submitForm" id="form1">
        <div class="mb-3">
            ...
            <small class="form-text text-danger" v-if="errors && errors.first_name">{{ errors.first_name[0] }}</small>
        </div>
        ...
        <div class="mb-3">
            ...
            <small class="form-text text-danger" v-if="errors && errors.last_name">{{ errors.last_name[0] }}</small>
        </div>

        <div class="mb-3">
            <label for="birthdate" class="form-label">Birth date:</label>
            <input type="date" class="form-control" name="birthdate" :max="maxDate" v-model="formData.birthdate">
            <small class="form-text text-danger" v-if="errors && errors.birthdate">{{ errors.birthdate[0] }}</small>
        </div>
        ...
        <div class="mb-3">
            ...
            <small class="form-text text-danger" v-if="errors && errors.report_subject">{{ errors.report_subject[0] }}</small>
        </div>
        ...
        <div class="mb-3">
            ...
            <span v-if="errors && formData.country" class="d-none">{{ formData.country[0] }}</span>
        </div>
        ...
        <div class="mb-3">
            ...
            <small class="form-text text-danger" v-if="errors && errors.phone">{{ errors.phone[0] }}</small>
        </div>
            ...
            axios.post('/submit_form1', this.formData)
                .then(response => {
                    if (response.status === 200) {
                        ...
                        window.location.href = `/step2?data=${data}`;
                    } else {
                        console.log('Error');
                    }
                })
